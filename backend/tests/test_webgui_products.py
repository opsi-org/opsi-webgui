...
depot_versions = {FQDN: "1.0-1", "pytest-test-depot.uib.gmbh": "1.0-1", "pytest-test-depot2.uib.gmbh": "2.0-1"}
...
async def test_products(config, input_data, expected_result):  # pylint: disable=too-many-arguments,redefined-outer-name
	res = requests.get(f"{config.external_url}{API_ROOT}/products", auth=(ADMIN_USER, ADMIN_PASS), verify=False, params=input_data)

	with open(expected_result, "r", encoding="utf-8") as f:
		json_string = (
			Template(f.read())
			.substitute(FQDN=FQDN, depots=depots, depot_versions=[x[1] for x in sorted(depot_versions.items())])
			.replace("'", '"')
		)
		print(json_string)
