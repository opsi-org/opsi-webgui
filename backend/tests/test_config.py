...
def test_config_uses_env_var_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_config_singleton_returns_same_instance(config_file: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_update_data_skips_reload_when_mtime_unchanged(config_instance: Config) -> None:
	...
	config_instance.config_file.write_text(yaml.dump({"ldap_auth": False}), encoding="utf-8")
	config_instance._mtime = 0.0
	config_instance._update_data()
	assert config_instance._data["ldap_auth"] is False

	# Overwrite but freeze mtime so cache thinks nothing changed.
	config_instance.config_file.write_text(yaml.dump({"ldap_auth": True}), encoding="utf-8")
...
def test_update_config_replace_mode(config_instance: Config) -> None:
	...
	config_instance.config_file.write_text(yaml.dump({"a": 1, "b": 2}), encoding="utf-8")
	config_instance._mtime = 0.0
	config_instance._update_data()
...
def test_get_ldap_config_dict_explicit_inactive_removes_sensitive_fields(
	...
	config_instance.config_file.write_text(
		yaml.dump({"ldap_auth": {"active": False, "ldap_url": "ldap://srv", "bind_user": "u"}}),
		encoding="utf-8",
	)
	result = config_instance.get_ldap_config()
...
def test_update_config_handles_write_error_gracefully(config_instance: Config, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_write_config_file_handles_permission_error(config_instance: Config, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_create_default_config_file_creates_file_with_defaults(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_log_returns_dict_for_dict_config(config_instance: Config) -> None:
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "level": "DEBUG"}}), encoding="utf-8")
	config_instance._mtime = 0.0
...
def test_get_log_custom_active_returns_true_when_active(
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True}}), encoding="utf-8")
...
def test_get_log_file_path_uses_configured_path(config_instance: Config, tmp_path: Path) -> None:
	"""get_log_file_path uses the configured log path."""
	log_dir = tmp_path / "logs"
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "path": str(log_dir)}}), encoding="utf-8")
	config_instance._mtime = 0.0
...
def test_get_log_file_path_uses_default_when_path_empty(
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "path": ""}}), encoding="utf-8")
	result = config_instance.get_log_file_path(create=False)
...
def test_get_log_level_defaults_to_info_when_invalid(config_instance: Config) -> None:
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "level": "INVALID"}}), encoding="utf-8")
	assert config_instance.get_log_level() == "INFO"
...
def test_get_log_level_defaults_to_info_when_empty_string(
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "level": ""}}), encoding="utf-8")
	assert config_instance.get_log_level() == "INFO"
...
def test_get_log_rotation_settings_handles_invalid_max_bytes(
	...
	config_instance.config_file.write_text(yaml.dump({"log": {"active": True, "rotate-max-bytes": -100}}), encoding="utf-8")
	max_bytes, backup_count = config_instance.get_log_rotation_settings()
...
def test_is_development_returns_true_for_development_env(
	...
	for env_value in ["development", "dev", "develop"]:
		config_instance.config_file.write_text(yaml.dump({"environment": env_value}), encoding="utf-8")
		config_instance._mtime = 0.0
...
def test_is_development_returns_false_for_production_env(
	...
	config_instance.config_file.write_text(yaml.dump({"environment": "production"}), encoding="utf-8")
	config_instance._mtime = 0.0
...
def test_get_log_detailed_returns_true_in_development(
	...
	config_instance.config_file.write_text(yaml.dump({"environment": "development"}), encoding="utf-8")
	config_instance._mtime = 0.0
