...
VOLATILE_KEYS = frozenset({"created", "lastSeen", "modificationTime", "opsiHostKey", "oneTimePassword"})


def remove_volatile(obj):
	if isinstance(obj, dict):
		return {key: remove_volatile(value) for key, value in obj.items() if key not in VOLATILE_KEYS}
...
def _get_config_server_id():
	...
	try:
		...
		cursor = mysql.cursor()
		cursor.execute('SELECT hostId FROM HOST WHERE type = "OpsiConfigserver" LIMIT 1;')
		row = cursor.fetchone()
