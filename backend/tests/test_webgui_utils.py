...
@pytest.mark.asyncio
async def test_filter_depot_access_none_selected_depots_uses_allowed(monkeypatch, request_with_user):
	monkeypatch.setattr(utils, "user_register", lambda: True)
	monkeypatch.setattr(utils, "depot_access_configured", lambda _user: True)
	monkeypatch.setattr(utils, "get_allowed_depots", lambda _user: ["depot-a", "depot-b"])
...
@pytest.mark.asyncio
async def test_filter_depot_access_filters_disallowed_depots(monkeypatch, request_with_user):
	monkeypatch.setattr(utils, "user_register", lambda: True)
	monkeypatch.setattr(utils, "depot_access_configured", lambda _user: True)
	monkeypatch.setattr(utils, "get_allowed_depots", lambda _user: ["depot-a", "depot-b"])
...
@pytest.mark.asyncio
async def test_filter_depot_access_keeps_empty_selection(monkeypatch, request_with_user):
	monkeypatch.setattr(utils, "user_register", lambda: True)
	monkeypatch.setattr(utils, "depot_access_configured", lambda _user: True)
...
@pytest.mark.asyncio
async def test_filter_depot_access_skips_functions_without_selected_depots(monkeypatch, request_with_user):
	...
	result = await reachable_clients(request=request_with_user, selectedClients=["client-1.opsi.org"])
	assert result == ["client-1.opsi.org"]


@pytest.mark.asyncio
async def test_filter_depot_access_injects_for_named_selected_depots_param(monkeypatch, request_with_user):
...
@pytest.mark.asyncio
async def test_filter_depot_access_supports_sync_functions(monkeypatch, request_with_user):
	...
	selected = await handler(request=request_with_user, selectedDepots=["depot-a", "forbidden"])
...
@pytest.mark.asyncio
async def test_filter_depot_access_no_user_register_passes_through(monkeypatch, request_with_user):
...
@pytest.mark.asyncio
async def test_filter_depot_access_no_depot_restriction_keeps_selection(monkeypatch, request_with_user):
	monkeypatch.setattr(utils, "user_register", lambda: True)
...
def test_get_objects_of_group_works_with_user_register_active(monkeypatch):
	...
	monkeypatch.setattr(
		...
		lambda gtype, parent_ids=None: (
			[{"group_id": "sub-group", "parent_id": "group-a", "type": gtype}] if parent_ids == ["group-a"] else []
		),
	)
	members = {
...
def test_get_objects_of_group_does_not_mutate_input(monkeypatch):
	...
	monkeypatch.setattr(utils, "_get_object_to_groups", lambda gtype, group_ids=None: [])
...
def test_group_get_all_clients_restricted_without_allowed_returns_empty(monkeypatch):
	...
	class _NoDbMySQL:
		def session(self):
			raise AssertionError("DB access must not happen when allowed client set is empty")
...
class TestGetBoolConfigValueDefaults:
	...
	def test_is_opsiserver_write_permitted_defaults_to_true_when_no_config(self, monkeypatch):
		self._make_db([], monkeypatch)
		assert utils.is_opsiserver_write_permitted("alice") is True

	def test_depot_access_configured_defaults_to_false_when_no_config(self, monkeypatch):
