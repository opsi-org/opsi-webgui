    ...
    for grp in groups:
        ...
        if grp["parent"] == group["id"]:
            if grp["id"] in processed:
                ...
                children[grp["id"]] = build_tree(
                    grp, groups, allowed, processed, default_expanded=default_expanded
                )
                if default_expanded and grp.get("hasAnySelection"):
                    group["hasAnySelection"] = True
    if children:
...
def host_group_access_configured(user: str) -> bool:
    return _get_bool_config_value(
        f"user.{{{user}}}.privilege.host.groupaccess.configured"
    )


def depot_access_configured(user: str) -> bool:
    return _get_bool_config_value(
        f"user.{{{user}}}.privilege.host.depotaccess.configured"
    )


def product_group_access_configured(user: str) -> bool:
    return _get_bool_config_value(
        f"user.{{{user}}}.privilege.product.groupaccess.configured"
    )


def read_only_user(user: str) -> bool:
    bak = (
        backend.accessControl_userIsReadOnlyUser()
    )  # if user is in readonly group in /etc/opsi/opsi.conf
    ur = _get_bool_config_value(
        f"user.{{{user}}}.privilege.host.all.registered_readonly"
    )  # if user roles read_only
    return bak or ur
...
def get_allowed_depots(user: str) -> list:
    with mysql.session() as session:
        where = text(
            "cv.configId='user.{" + user + "}.privilege.host.depotaccess.depots'"
        )
        where = and_(where, text("cv.isDefault=1"))
        query = (
            select(text("cv.value"))
            .select_from(text("CONFIG_VALUE AS cv"))
            .where(where)
        )
        result = session.execute(query)
...
def get_allowed_product_groups(user: str) -> list:
    with mysql.session() as session:
        where = text(
            "cv.configId='user.{"
            + user
            + "}.privilege.product.groupaccess.productgroups'"
        )
        where = and_(where, text("cv.isDefault=1"))
        query = (
            select(text("cv.value"))
...
def _get_object_to_groups(
    gtype: str, group_ids: list[str] | str | None = None
) -> list[str]:
...
def get_objects_of_group(
    ...
    for row in child_groups:
        ...
        objs_ids = [
            obj["objectId"]
            for obj in _get_object_to_groups(group_type, group_ids=row["group_id"])
        ]
...
def get_allowed_host_groups(user: str) -> list:
    with mysql.session() as session:
        where = text(
            "cv.configId='user.{" + user + "}.privilege.host.groupaccess.hostgroups'"
        )
        where = and_(where, text("cv.isDefault=1"))
        query = (
            select(text("cv.value"))
...
def get_groups_ids(type: str) -> list[str]:
    ...
    with mysql.session() as session:
        query = (
            select(text("g.groupId AS group_id"))
            .select_from(table("GROUP").alias("g"))
            .where(text("g.type = :type"))
