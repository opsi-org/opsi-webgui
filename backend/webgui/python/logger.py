...
mysql = get_mysql()
PRINT_TO_CUSTOM_LOG_FILE_DEFAULT = True  # warning and errors always printed to custom logfile
...
def get_clients_of_server(opsi_backend: Any, server_id: str) -> dict[str, str]:
	...
	if not server_id or server_id == "":
		_client_server: list[dict[str, str]] = opsi_backend.configState_getClientToDepotserver()
		_client2server: dict[str, str] = {obj["clientId"]: obj["depotId"] for obj in _client_server}
	...
	res = {obj["clientId"]: obj["depotId"] for obj in opsi_backend.configState_getClientToDepotserver(depotIds=[server_id])}
	return res
...
class Logger(logging.Logger):
	...
	def __log(
		...
		*args,
		...
		with log_context({"action": ADDON_ID}):
			...
			if with_event and self.local_config and self.local_config.get_log_custom_active():
	...
	def detailed(self, msg: object, *args: Any, **kwargs: Any) -> None:
		...
		if hasattr(self.local_config, "get_log_detailed") and self.local_config.get_log_detailed():
			self.__log(logging.TRACE, msg, with_event, *args, **kwargs)
	...
	def devel(self, msg: object, *args: Any, **kwargs: Any) -> None:
		...
		if hasattr(self.local_config, "get_log_devel") and self.local_config.get_log_devel():
...
def get_logger() -> Logger:
	...
	try:
		...
		_logger.error(f"Failed to initialize logger in get_logger: {e}", with_event=True)
		pass
