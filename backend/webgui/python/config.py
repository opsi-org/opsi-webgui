...
ENV_KEY_LDAP_URL = (
    "OPSI_TEST_LDAP_URL"  # Environment variable key for LDAP URL override
)
...
class Config(metaclass=Singleton):  # pylint: disable=too-few-public-methods
    ...
    def _update_data(self, _logger=None):
        if not self.config_file.exists() and not AUTO_CREATE_CONFIG:
            ...
            logger.warning(
                f"Config file '{self.config_file}' does not exist. Creating default config file."
            )
            return
    ...
    def get_ldap_config(
        ...
        if isinstance(ldap_auth, bool) and ldap_auth:
            ...
            ldap_config_res = {
                str(k): v if isinstance(v, (bool, int, list)) else str(v)
                for k, v in ldap_auth.items()
            }
            ldap_config_res["groups"] = self._get_ldap_auth_groups()
    ...
    def _get_ldap_auth_groups(
        self, default_groups: list[str] | None = None
    ) -> list[str]:
