...
def get_host_data(
	...
	with mysql.session() as session:
		query = (
			...
			IF(
				COALESCE((SELECT cs.values FROM CONFIG_STATE cs WHERE cs.objectId = h.hostId AND cs.configId = 'opsiclientd.event_timer.active'), (SELECT cv.value FROM CONFIG_VALUE cv WHERE cv.configId = 'opsiclientd.event_timer.active' AND cv.isDefault)) LIKE '%true%'
