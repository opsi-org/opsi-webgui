    ...
    with mysql.session() as session:
        ...
        for row in result:
            if row is not None:
                ...
                for key in row_dict:
                    if isinstance(
                        row_dict.get(key), (datetime.date, datetime.datetime)
                    ):
                        row_dict[key] = row_dict.get(
                            key, datetime.datetime(2000, 1, 1, 0, 0)
                        ).isoformat()
...
def get_host_groups(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    if selectedDepots == []:
        ...
        return RESTResponse(
            data={"groups": empty_groups, "clientdirectory": empty_clientdirectory}
        )
    ...
    return RESTResponse(
        data={"groups": host_groups, "clientdirectory": clientdirectory}
    )
...
def get_host_groups_dynamic(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    if parentGroup == "root" or not parentGroup:
        parentGroup = "root"
        where = and_(
            where, text("g.parentGroupId IS NULL AND g.groupId = 'clientdirectory'")
        )  # type: ignore
    ...
    with mysql.session() as session:
        if (
            recursiveMembers
            and parentGroup
            and parentGroup not in ("root", "not_assigned")
        ):
            ...
            if restricted:
                actual_group_ids = [
                    ...
                    or group_id.lower() in (allowed_children or set())
                ]
            ...
            if parentGroup == "clientdirectory":
                ...
                members = sorted(
                    set(members).union(
                        group_get_all_clients("clientdirectory", params["depots"])
                    )
                )
            return RESTResponse(
                data={
        ...
        if parentGroup and parentGroup not in ("root", "not_assigned"):
            ...
            for row in child_group_rows:
                ...
                if (
                    restricted
                    and group_id.lower() not in (allowed_children or set())
        ...
        if parentGroup and parentGroup != "root":
            ...
            all_groups = read_groups(
                result, root_group, selectedClients, allowed, withClients
            )
        ...
        if selectedClients:
            ...
            host_groups = build_tree(
                root_group, list(all_groups.values()), allowed, default_expanded=True
            )
...
def group_get_all_clients(group: str, depots: list | None = None) -> list:
    ...
    with mysql.session() as session:
        ...
        query = (
            select(text("h.hostId AS clientId"))
            .select_from(table("HOST").alias("h"))
            .where(where)
        )
...
def get_server_data(
    ...
    with mysql.session() as session:
		...
		"""
            ...
            .select_from(table("HOST").alias("h"))
            .where(
                and_(where, text("h.type IN ('OpsiDepotserver','OpsiConfigserver') "))
            )
        )  # pylint: disable=redefined-outer-name
...
def host_check_duplicates(host: Host, session: Any) -> None:
    if (
        mysql.unique_hardware_addresses
        ...
        select_query = (
            ...
            .where(
                text(
                    f"h.hostId != '{host.hostId}' AND hardwareAddress = '{host.hardwareAddress}'"
                )
            )
        )  # pylint: disable=redefined-outer-name
        ...
        if result:
            raise BackendBadValueError(
                f"Hardware address {host.hardwareAddress!r} is already used by host {result}"
            )
