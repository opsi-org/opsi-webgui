...
def get_host_groups_dynamic(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
	...
	with mysql.session() as session:
		if recursiveMembers and parentGroup and parentGroup not in ("root", "not_assigned"):
			...
			if allowed and parentGroup not in allowed and parentGroup != "clientdirectory":
				return RESTResponse(
					data={
						"groups": {"id": parentGroup, "type": "HostGroup", "text": parentGroup, "parent": None, "children": {}},
						"members": [],
					}
				)
			...
			if allowed:
				...
				actual_group_ids = [
					group_id for group_id in actual_group_ids if group_id == "clientdirectory" or group_id.lower() in allowed_normalized
				]
			...
			if parentGroup == "clientdirectory":
				...
				members = sorted(set(members).union(group_get_all_clients("clientdirectory", selectedDepots)))
			return RESTResponse(
				data={
					"groups": {
						"id": parentGroup,
		...
		if parentGroup and parentGroup not in ("root", "not_assigned"):
			...
			for row in child_group_rows:
				...
				if not group_id:
					continue
				if allowed and group_id not in allowed and group_id != "clientdirectory":
					continue
