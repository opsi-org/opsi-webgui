	...
	with mysql.session() as session:
		if recursiveMembers and parentGroup and parentGroup != "groups":
			...
			if allowed and parentGroup not in allowed and parentGroup != "groups":
				return RESTResponse(
					data={
						"groups": {"id": parentGroup, "type": "ProductGroup", "text": parentGroup, "parent": None, "children": {}},
						"members": [],
					}
				)
			...
			if allowed:
				...
				actual_group_ids = [group_id for group_id in actual_group_ids if group_id.lower() in allowed_normalized]
