...
def get_product_description(
    product: str, product_version: str, package_version: str
) -> tuple[str, str]:
    description = None
    params = {}
    with mysql.session() as session:
        ...
        where = text(
            "p.productId = :product AND p.productVersion = :product_version AND p.packageVersion = :package_version"
        )

        query = (
            select(text("description, advice"))
            .select_from(table("PRODUCT").alias("p"))
            .where(where)
        )
...
def get_product_type(
    product_id: str, product_version: str, package_version: str
) -> str | None:
    with mysql.session() as session:
        query = (
            select(text("type"))
            ...
            .where(
                text(
                    "productId = :product_id AND productVersion = :product_version AND packageVersion = :package_version"
                )
            )
        )
...
def get_product_actions(product: str, version: str, package_version: str) -> list[str]:
    ...
    params["package_version"] = package_version
    where = text(
        "productId = :product AND productVersion = :version AND packageVersion = :package_version"
    )
...
def is_product_on_depot(
    product: str, version: str, package_version: str, depot: str
) -> bool:
    params = {}
    params["product"] = product
    params["version"] = version
...
def products(  # pylint: disable=too-many-locals, too-many-branches, too-many-statements, redefined-builtin, invalid-name, unused-argument, too-many-arguments
    ...
    with mysql.session() as session:
        ...
        for row in result:
            ...
            product["depot_version_diff"] = bool(
                product.get("depot_version_diff", False)
            )
            product["client_version_outdated"] = bool(
        ...
        total = session.execute(
            select(text("COUNT(*)")).select_from(products_on_depots), params
        ).fetchone()[0]
    ...
    for client_id in data.clientIds:
        ...
        for product_id in data.productIds:
            ...
            if product_id not in depot_product_version[depot_id]:
                depot_product_version[depot_id][product_id] = depot_get_product_version(
                    depot_id, product_id
                )
            if not depot_product_version[depot_id][product_id]:
                http_status = status.HTTP_400_BAD_REQUEST
                result_data[client_id][product_id] = (
                    f"Product '{product_id}' not available on depot '{depot_id}'."
                )
                continue
            ...
            if package_version not in product_actions[product_id][product_version]:
                product_actions[product_id][product_version][package_version] = (
                    get_product_actions(product_id, product_version, package_version)
                )
    ...
    with mysql.session() as session:
        try:  # pylint: disable=too-many-nested-blocks
            ...
            for row in result:
                if row is not None:
                    ...
                    for depot in _depots:
                        ...
                        property["multiValueDetails"][depot] = bool(
                            property["multiValue"]
                        )
                        property["editableDetails"][depot] = bool(property["editable"])

                        if property["type"] == "BoolProductProperty":
                            property["allValues"].update(
                                [
                                    bool_value(value)
                                    for value in property["values"].split(",")
                                ]
                            )
                            if isinstance(defaults, dict):
                                property["defaultDetails"][depot] = [
                                    bool_value(defaults.get(depot))
                                ]
                            ...
                            property["possibleValues"][depot] = [
                                bool_value(value)
                                for value in property["values"].split(",")
                            ]
                        else:
                            property["allValues"].update(
                                unicode_value(property["values"])
                            )
                            property["defaultDetails"][depot] = unicode_value(defaults)
                            property["possibleValues"][depot] = unicode_value(
                                property["values"]
                            )
						...
						"""
                            ...
                            .where(
                                text(
                                    "pps.productId = :product AND pps.propertyId = :property AND pps.objectId = :depot"
                                )
                            )
                        )
                        ...
                        if values is not None:
                            if property["type"] == "BoolProductProperty":
                                property["depots"][depot] = [
                                    bool_value(dict(values).get("values", ""))
                                ]
                                property["allValues"].update(
                                    [bool_value(dict(values).get("values", ""))]
                                )
                            else:
                                property["depots"][depot] = unicode_value(
                                    dict(values).get("values", "")
                                )
                                property["allValues"].update(
                                    unicode_value(dict(values).get("values", ""))
                                )
                            if (
                                property["depots"][depot]
                                != property["defaultDetails"][depot]
                            ...
                            property["depots"][depot] = property["defaultDetails"][
                                depot
                            ]
                        ...
                        for client in clients_on_depot.get(depot, []):
							...
							"""
                                ...
                                .where(
                                    text(
                                        "pps.productId = :product AND pps.propertyId = :property AND pps.objectId = :client"
                                    )
                                )
                            )
                            values = session.execute(
                            ...
                            if values is not None:
                                if property["type"] == "BoolProductProperty":
                                    ...
                                    property["clients"][client] = unicode_value(
                                        dict(values).get("values", "")
                                    )
                                    property["allValues"].update(
                                        unicode_value(dict(values).get("values", ""))
                                ...
                                property["clients"][client] = property[
                                    "defaultDetails"
                                ][depot]
                    ...
                    data["properties"][property["propertyId"]] = merge_dicts(
                        property, data["properties"][property["propertyId"]]
                    )
            ...
            for depot in clients_on_depot:
                data["productVersions"][depot] = depot_get_product_version(
                    depot, productId
                )
            ...
            if data["productDescriptionDetails"]:
                if all(
                    ...
                    data["productDescription"] = list(
                        data["productDescriptionDetails"].values()
                    )[0]
                else:
            ...
            if data["productAdviceDetails"]:
                if all(
                    ...
                    data["productAdvice"] = list(data["productAdviceDetails"].values())[
                        0
                    ]
                else:
...
def get_product_product_property_state(
    object_id: str, product_id: str, property_id: str
) -> str | None:
    with mysql.session() as session:
        query = (
            select(
		...
		"""
            ...
            .where(
                text(
                    "productId = :product_id AND objectId = :object_id AND propertyId = :property_id"
                )
            )
        )
...
def save_poduct_property(  # pylint: disable=invalid-name, too-many-locals, too-many-statements, too-many-branches, unused-argument
    ...
    with mysql.session() as session:
        for object_id in objects:
            ...
            if depot_id not in depot_product_version:
                depot_product_version[depot_id] = {}
                depot_product_version[depot_id][productId] = depot_get_product_version(
                    depot_id, productId
                )
            ...
            for property_id in data.properties:
                if property_id not in available_properties:
                    logger.error(
                        "Propertiy %s does not exist on %s.", property_id, depot_id
                    )
                    raise OpsiApiException(
                ...
                try:
                    if get_product_product_property_state(
                        object_id, productId, property_id
                    ):
                        stmt = (
                            update(
                                table(
                            ...
                            .where(
                                text(
                                    f"productId = '{productId}' AND objectId = '{object_id}' AND propertyId = '{property_id}'"
                                )
                            )
                            .values(**values)
                        )
...
def installation_status(request: Request) -> RESTResponse:  # pylint: disable=unused-argument
    ...
    with mysql.session() as session:
        try:
            query = (
                select(text("poc.installationStatus"))
            ...
            installation_status_list = [
                dict(row).get("installationStatus") for row in result
            ]
...
def get_product_groups_dynamic(
    ...
    with mysql.session() as session:
        if recursiveMembers and parentGroup and parentGroup != "groups":
            ...
            descendant_group_ids = get_all_children_groupids(
                raw_group_rows, [parentGroup.lower()]
            )
            normalized_to_actual = {
                str(row["group_id"]).lower(): str(row["group_id"])
                for row in raw_group_rows
                if row.get("group_id")
            }
            actual_group_ids = [
                normalized_to_actual[group_id]
                for group_id in descendant_group_ids
                if group_id in normalized_to_actual
            ]
            ...
            member_rows = session.execute(
                ...
                .where(
                    and_(text("og.groupType = 'ProductGroup'"), or_(*member_conditions))
                ),
                member_params,
            ).fetchall()
...
def delete_product_group(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    try:
        ...
        logger.error(error)
        return RESTErrorResponse(
            message=f"Could not delete group {group}.", details=error
        )
...
def update_product_group(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    try:
        ...
        logger.error(error)
        return RESTErrorResponse(
            message=f"Could not update group {group}.", details=error
        )
...
def rm_product_from_product_group(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    try:
        backend.objectToGroup_delete(
            groupType="ProductGroup", objectId="*", groupId=group
        )
    except Exception as error:  # pylint: disable=broad-exception-caught
        logger.error(error)
        return RESTErrorResponse(
            message=f"Could not delete products from group {group}.", details=error
        )
...
def rm_product_from_product_group(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    try:
        backend.objectToGroup_delete(
            groupType="ProductGroup", objectId=product, groupId=group
        )
    except Exception as error:  # pylint: disable=broad-exception-caught
        logger.error(error)
        return RESTErrorResponse(
...
def create_product_group(  # pylint: disable=invalid-name, too-many-locals, too-many-branches, too-many-statements
    ...
    with mysql.session() as session:
        try:
            ...
            return RESTResponse(
                data=values, http_status=status.HTTP_201_CREATED, headers=headers
            )
