...
async def clients(  # pylint: disable=too-many-branches, dangerous-default-value, invalid-name, unused-argument, too-many-locals
    ...
    with mysql.session() as session:
        ...
        if backend._host_control_use_messagebus is True or (
            ...
            and "reachable" in commons.get("sortBy", [])
        ):
        ...
        total = session.execute(
            select(text("COUNT(*)")).select_from(client_with_depot), params
        ...
        for row in result:
            if row is not None:
                ...
                client["reachable"] = (
                    bool(client["reachable"])
                    if client["reachable"] is not None
                    else None
                )
                client["selected"] = (
                    bool(client["selected"]) if client["selected"] is not None else None
                )
                data.append(client)
...
def _depots_of_clients(clients: list[str] | None) -> dict:
    ...
    with mysql.session() as session:
        ...
        clients = [
            dict(row)["hostId"] for row in session.execute(query, params).fetchall()
        ]
        ...
        query = (
            select(text("cs.objectId AS client, cs.values"))
...
@check_client_creation_rights
def create_client(
    request: Request, client: Client, depot: str = Body(default="")
) -> RESTResponse:  # pylint: disable=too-many-locals
    """
    Create OPSI-Client.
    """
    ...
    try:
        ...
        if depot:
            backend.configState_create(
                configId="clientconfig.depot.id", objectId=client.hostId, values=[depot]
            )
...
def update_client(request: Request, client_id: str, client: Client) -> RESTResponse:  # pylint: disable=too-many-locals
    ...
    try:
        ...
        return RESTResponse(
            data=client.__dict__, http_status=status.HTTP_201_CREATED, headers=headers
        )
...
async def host_control_process_action(
    request: Request, data: ProcessActionRPC
) -> RESTResponse:  # pylint: disable=unused-argument
    """
...
async def deploy_client_agent(clientDeployData: ClientDeployData) -> RESTResponse:  # pylint: disable=invalid-name
    ...
    if clientDeployData.type == "linux":
        deploy_script = (
            "/var/lib/opsi/depot/opsi-linux-client-agent/opsi-deploy-client-agent"
        )
    if clientDeployData.type == "macos":
        deploy_script = (
            "/var/lib/opsi/depot/opsi-mac-client-agent/opsi-deploy-client-agent"
        )
    ...
    logger.warning(
        "It looks like the client agent (%s) is not installed.", clientDeployData.type
    )
...
def rm_client_from_groups(
    ...
    if not groups:
        ...
        return RESTErrorResponse(
            http_status=status.HTTP_400_BAD_REQUEST, message="No group given."
        )

    try:
        for group in groups:
            backend.objectToGroup_delete(
                groupType="HostGroup", groupId=group, objectId=clientid
            )
        return RESTResponse(
            http_status=200,
...
def blocked_clients(request: Request) -> RESTResponse:  # pylint: disable=unused-argument
    ...
    with redis_client() as redis:
        ...
        for key in redis_keys:
            blocked_client_list.append(
                ip_address_from_redis_key(key.decode("utf8").split(":")[-1])
            )
...
def unblock_all_clients(request: Request) -> RESTResponse:  # pylint: disable=unused-argument
    ...
    try:
        with redis_client() as redis:
            ...
            return RESTResponse(
                {"clients": list(client_set), "redis-keys": list(deleted_keys)}
            )
    except Exception as err:  # pylint: disable=broad-except
    ...
    try:  # pylint: disable=too-many-nested-blocks
        ...
        if product_action.outdated:
            ...
            for pod in backend.productOnDepot_getObjects(depotId=depots):
                ...
                depot_versions[pod.depotId][pod.productId] = (
                    f"{pod.productVersion}-{pod.packageVersion}"
                )
            result = set(
                backend.productOnClient_getObjects(
                    installationStatus="installed", clientId=hosts
                )
            )
            for poc in result.difference(poc_list):
                ...
                try:
                    ...
                    ) < version.parse(depot_versions[depot][poc.productId]):
                        logger.info(
...
@check_client_creation_rights
def clone_client(
    request: Request, client_id: str, target: Target, options: CloneOptions
) -> RESTResponse:  # pylint: disable=too-many-locals
    """
    Clone OPSI-Client.
    """
