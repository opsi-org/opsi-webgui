...
class Authentication:
	...
	def _get_config(self):
		...
		if _active and _active_opsiconfig:
			...
			_config = {
				...
				"use_member_of_rdn": bool(self.config_ldap.get("use_member_of_rdn", False)),
			}
		else:
			logger.debug(f"{TAG} LDAP authentication not active. Using default authentication.")
	...
	async def authenticate(self, connection: HTTPConnection, receive: Receive) -> None:
		try:
			if self.config_ldap.get("active", False):
				...
				logger.debug(f"{TAG} LDAP authentication not active, falling back to backend auth")
				await self.__authenticate_backend(connection, receive)
		except Exception as err:
			logger.error(f"{TAG} Authentication failed: {err}", with_event=True)
			raise HTTPException(
				status_code=(err.status_code if isinstance(err, HTTPException) else status.HTTP_401_UNAUTHORIZED),
				detail=err.detail if isinstance(err, HTTPException) else "Unauthorized",
			) from err

	async def __authenticate_ldap(self, connection: HTTPConnection, receive: Receive) -> bool:
		if not self.config_ldap.get("active", False) or (
			not self.config_ldap.get("opsiconfig", False) and not self.config_ldap.get("ldap_url")
		):
			raise HTTPException(
		...
		logger.debug(f"{TAG} Starting LDAP authentication for session '{session.session_id}'")
		logger.detailed(f"{TAG} Session data at start of LDAP authentication: {session.__dict__}")
		...
		if groupnames:
			...
			logger.detailed(f"{TAG} Session data after LDAP authentication: {session.__dict__}")
			return True

		return False

	async def __authenticate_backend(self, connection: HTTPConnection, receive: Receive) -> None:
	...
	async def _get_credentials(self, connection: HTTPConnection, receive: Receive, log_errors: bool = True) -> tuple[str, str, str | None]:
		errors = []
		try:
		...
		if log_errors:
			logger.error(f"Failed to get credentials. Errors: {errors}", with_event=True)
		raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid credentials")

	async def __get_credentials_form_data(self, connection: HTTPConnection, receive: Receive) -> tuple[str, str, str | None]:
		...
		logger.debug(f"{TAG} Retrieved credentials from form data for user '{username}'")
		return username, password, mfa_otp

	async def __get_credentials_http_basic(self, connection: HTTPConnection, receive: Receive) -> tuple[str, str, str | None]:
		creds = await HTTPBasic(auto_error=False)(Request(connection.scope, receive))
		if creds and getattr(creds, "username", None) and getattr(creds, "password", None):
			logger.debug(f"{TAG} Retrieved credentials from HTTP Basic auth for user '{creds.username}'")
	...
	async def credentials_provided(self, connection: HTTPConnection, receive: Receive) -> bool:
		try:
			...
			pass
		logger.trace(f"{TAG} No credentials provided in request (session-based auth possible)")
		return False
	...
	def _check_group_membership(self, usergroups: list[str], info: str = "") -> bool:
		...
		if not user_groups_set.intersection(required_groups_set):
			...
			logger.debug(
				f"{TAG} User groups '{usergroups}' do not include any of the required groups '{required_groups}'{info}",
				with_event=True,
			)
			return False

		logger.debug(f"{TAG} User is member of required group(s): {user_groups_set.intersection(required_groups_set)}")
		return True

	async def authenticated(self, connection: HTTPConnection | None = None, receive: Receive | None = None) -> bool:
		session = await ensure_session(scope=connection.scope) if connection else await ensure_session()
		if session is None:
			logger.warning(f"{TAG} No session found for request", with_event=True)
			return False
		logger.debug(f"{TAG} Validating authentication for session '{session.session_id}'")

		logger.detailed(f"{TAG} Session data for authentication check: {session.__dict__}")
		if not session.authenticated:
			...
			await post_authenticate(connection.scope)
			return False
		logger.debug(f"{TAG} Active session for user '{session.username}' with groups {session.user_groups}")
		if not self._check_group_membership(session.user_groups, "from session check"):
			logger.warning(f"{TAG} User does not belong to the required group", with_event=True)
			session.authenticated = False
			await session.store(wait=True)
			await post_authenticate(connection.scope)
			return False
