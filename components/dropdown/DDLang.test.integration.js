...
test('dropdown lang snapshot', async ({ page }) => {
  ...
  expect(await component.screenshot()).toMatchSnapshot('DDLang-normal.png', { threshold: 0.2 })
})
...
test('dropdown lang opened snapshot', async ({ page }) => {
  ...
  expect(await component.screenshot()).toMatchSnapshot('DDLang-opened.png', { threshold: 0.2 })
})
...
test('dropdown lang changed snapshot', async ({ page }) => {
  ...
  const component = await page.locator('[data-testid="DropdownDDLang"]')
  expect(await component.screenshot()).toMatchSnapshot('DDLang-changed.png', { threshold: 0.2 })
})
