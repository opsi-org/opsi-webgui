...
<template>
  ...
  <el-popover v-if="isProductGroup" :placement="popoverPlacement" trigger="click" :width="popoverWidth"
    ...
    <template v-if="isCreateGroupPopoverLoaded">
      <el-form label-position="top" class="mt-3">
        ...
        <el-form-item v-for="label in Object.keys(createGroup)" :key="label" :label="$t(label)"
          v-show="label !== 'parentGroupId'">
          <el-input v-model="createGroup[label]" @keyup.enter="createSubGroup('')" />
        </el-form-item>
        <el-button class="float-right" type="success" data-testid="createSubGroup" @click="createSubGroup('')"
          :disabled="!createGroup.groupId">{{ $t('create') }}</el-button>
      </el-form>
    </template>
  </el-popover>
  <el-container v-loading="isLoading">
    <el-tree :ref="props.data.category" :class="treeClass" :data="fetchedData" :props="treeProps" node-key="id"
      ...
      <template #default="{ node, data: defdata }">
        ...
        <div class="ml-auto" v-if="node.label !== 'not_assigned'">
          <span :key="node.label + action" v-for="action in getActions(defdata, node)">
            <el-popover :placement="popoverPlacement" :width="popoverWidth" trigger="click" :ref="node.label + action"
              @show="loadActionPopover(node.label + action)">
              <template #reference>
                <el-button size="small" :disabled="config.read_only">
                  <IconIIcon v-for="subaction in action.split('-')" :key="subaction"
              ...
              <template v-if="isActionPopoverLoaded[node.label + action]">
                ...
                <el-form label-position="top" class="mt-3">
                  <template v-if="action === 'group-add'">
                    <el-form-item v-for="label in Object.keys(createGroup)" :key="label" :label="$t(label)"
                      v-show="label !== 'parentGroupId'">
                      <el-input v-model="createGroup[label]" @keyup.enter="createSubGroup(node.label)" />
                    </el-form-item>
                    <el-button class="float-right" type="success" data-testid="createSubGroup"
                      @click="createSubGroup(node.label)" :disabled="!createGroup.groupId || config.read_only">{{
                        $t('create') }}</el-button>
                  </template>
                  <template v-else-if="['client-add', 'product-add'].includes(action)">
                    ...
                    <el-button class="float-right" type="success" data-testid="addChildren" :disabled="config.read_only"
                      @click="addChildren(node.label)">{{ $t('add') }}</el-button>
                  </template>
                  <template v-else-if="['client-delete', 'product-delete'].includes(action)">
                    <el-text>{{ $t(action + '.confirm') }}</el-text>
                    <el-button class="float-right" type="danger" :disabled="config.read_only"
                      data-testid="removeAssignments" @click="deleteAllChildren(node.label)">{{ $t('delete')
                      }}</el-button>
                  </template>
                  <template v-else-if="action === 'delete'">
                    <el-text>{{ $t(action + '.confirm') }}</el-text>
                    <el-button type="danger" class="float-right" :disabled="config.read_only"
                      @click="applyDelete(node.label, defdata.type, defdata.parent)">{{ $t('delete') }}</el-button>
                  </template>
                  <template v-else-if="action === 'edit'">
                    <el-form-item v-for="label in Object.keys(editgroup)" :key="label" :label="$t(label)">
                      <el-select v-if="label === 'parent'" v-model="editgroup[label]" :teleported="false">
                        <!-- if item is (nested) child of node. Do not allow -->
                        <el-option v-for="item in filteredGroupNames.filter((name) => {
                          return (
                            name !== node.label &&
                            name !== NOT_ASSIGNED &&
                            !getChildNodes(node).includes(name)
                          )
                        })" :key="item" :label="item" :value="item" :disabled="node.data.parent == item" />
                      </el-select>
                      <el-input v-else v-model="editgroup[label]" />
                    </el-form-item>
                    <el-button class="float-right" type="success" data-testid="editGroup" :disabled="config.read_only"
                      @click="updateGroup(node.label)">{{ $t('update') }}</el-button>
                  </template>
                  <template v-else-if="action === 'copy'">
                    ...
                    <el-button type="success" class="float-right" :disabled="config.read_only"
                      @click="copyClient(node.label)">{{ $t('copy') }}</el-button>
                  </template>
