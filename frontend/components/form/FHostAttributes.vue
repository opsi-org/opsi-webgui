...
<template>
  <div data-testid="FHostAttributes">
    ...
    <el-form v-if="hostAttributes.length && hostAttributes[0]" label-width="50%"
      :label-position="mq.isMobile.value ? 'top' : 'left'" v-loading="isLoading">
        ...
        <div v-for="(value, label) in hostAttributes[0]" :key="label">
          <el-form-item :label="`${$t('table.fields.' + label)}`">
            <el-checkbox v-if="typeof value === 'boolean'" v-model="hostAttributes[0][label]" :value="value"
              :disabled="notEditable.includes(label) || config.read_only" @change="setUnsavedChanges" />
            <el-input v-else-if="isInputPasswordLabel(label)" v-model="hostAttributes[0][label]" :value="value"
              show-password :disabled="notEditable.includes(label) || config.read_only" @input="setUnsavedChanges" />
            <el-input v-else-if="isInputDateLabel(label)" :value="useFormat().date(value)"
              :disabled="true || config.read_only" />
            <el-input v-else v-model="hostAttributes[0][label]" :value="value"
              :disabled="notEditable.includes(label) || config.read_only" @input="setUnsavedChanges" />
          </el-form-item>
        </div>
      </div>

      <div v-if="hostAttributes[0].type !== 'OpsiDepotserver' && !config.read_only" class="button-container"
