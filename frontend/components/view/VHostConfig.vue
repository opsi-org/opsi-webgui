...
<template>
  <div id="config-pre-tabs">
    ...
    <!-- reload data -->
    <el-button @click="refetch" :title="$t('label.reload')">
      <IconIIcon :icon="useIcons().refetch" />
    </el-button>
  </div>
  <el-tabs v-model="activeName">
    <el-tab-pane :label="currentId ? $t('title.hostparam') : $t('title.hostparam.defaults')" name="config"
      :disabled="!(type === 'clients' || type === 'servers')">
      <FormFHostParameter ref="refHostParam" v-if="activeName === 'config'" :id="currentId" :type="type"
        :is-child="props.isChild" />
    </el-tab-pane>
    <el-tab-pane :label="$t('title.hostattr')" name="attr" :disabled="isIdEmpty">
      <FormFHostAttributes ref="refHostAttr" v-if="activeName === 'attr'" :id="currentId" :type="type"
        :is-child="props.isChild" />
    </el-tab-pane>
  </el-tabs>
</template>
