...
<script setup lang="ts">
  ...
  const items = ref([
    {
      id: '_header',
      label: $t('table.contextmenu.header-specific', { id: 'XXX' }),
      disabled: true /* row ident */,
    },

    {
      id: 'action_ondemand_all',
      label: $t('button.event.ondemand'),
      icon: icons.ondemand,
      command: () => call_opsievent('ondemand-all'),
      visible: props.type !== 'clients',
    },
    {
      id: '_actions',
      label: $t('button.item-actions'),
      icon: icons.menu,
      visible: props.type === 'clients',
      items: [
        {
          id: 'action_ondemand',
          label: $t('button.event.ondemand'),
          command: () => call_opsievent('ondemand'),
        },
        {
          id: 'action_showpopup',
          label: $t('button.event.showpopup'),
          command: () => call_opsievent('showpopup'),
        },
        {
          id: 'action_reboot',
          label: $t('button.event.reboot'),
          command: () => call_opsievent('reboot'),
        },
        {
        ...
        },
        {
          ...
          command: () => call_opsievent('rename'),
        },
        {
          id: 'action_delete',
          label: $t('label.delete'),
          command: () => call_opsievent('delete'),
        },
      ],
    },
    {
      ...
      icon: icons.settings,
      command: () => call_navigate('config'),
    },
    {
      id: 'to_log',
      label: $t('title.log'),
      icon: icons.log,
      command: () => call_navigate('logs'),
    },
    ...
    {
      id: 'table_sort',
      label: $t('button.sort.tablecolumns'),
      icon: icons.sort,
      items: [],
    },
    {
      id: 'table_showcol',
      label: $t('table.showCol'),
      icon: icons.columns,
      items: [],
    },
  ...
  function call_navigate(pagetype: string) {
    if (props.item === undefined) throw new Error('itemModel is undefined [2]')
