...
<script setup lang="ts">
  ...
  const possibleColumnsModel = defineModel<ITableHeaderRow>('possibleColumns', {
    required: true,
  })
  ...
  const selectedColumnsIds = ref(
    useUtilsData().getVisibleColumnIdsInSelect(possibleColumnsList.value),
  )
  watch(
    () => possibleColumnsModel,
    () => {
      selectedColumnsIds.value = useUtilsData().getVisibleColumnIdsInSelect(
        possibleColumnsList.value,
      )
    },
    { deep: true },
  )
  ...
  const _options = computed<Array<ITableHeaderCell>>(() => {
    return possibleColumnsList.value.filter(
      (h: ITableHeaderCell) =>
        h._majorKey === undefined || h._isMajor !== undefined, // do not show child columns in select
    )
  })

  const optionsWrapper = computed<Array<any>>(() => {
    // need another format for el-select-v2
    return _options.value.map((v) => ({
      value: v.key,
  ...
  function handleItem(selections: Array<string>) {
    ...
    for (const item of possibleColumnsList.value) {
      ...
      if (!entry.isMajor) {
        ...
        const children = Object.values(possibleColumnsModel.value).filter(
          (c: ITableHeaderCell) => c._majorKey === key,
        )
