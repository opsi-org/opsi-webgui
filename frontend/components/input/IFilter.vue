<template>
  ...
  <el-form class="max-w-md">
    <el-input v-model="filterQueryValue" :placeholder="data.filterQuery">
      <template #prepend>
        <el-select-v2
          v-model="filterValue"
          :options="
            props.filterableColumns.map((v: any) => ({
              value: v.key,
              label: v.title,
          ...
          <template #default="{ item }">
            <div class="inline">
              <IconIIcon
                class="inline"
                :icon="icon.check"
                v-if="filterValue.includes(item.value)"
              />
...
<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    filterableColumns: {
      type: Object as PropType<Array<ITableHeaderRow>>,
      required: true,
    },
