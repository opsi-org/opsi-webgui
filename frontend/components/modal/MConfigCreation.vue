...
<template>
  <div>
    <p-dialog v-model:visible="visible" modal :header="$t('title.creation.config')" :style="{ width: '25rem' }">
      <!-- Name -->
      <div class="flex flex-col gap-1 pb-2">
        <div class=".k-name">
          <label for="name">{{ configdata.name.label }}</label>
          <p-input-text v-if="configdata.name.type === 'text'" id="name" type="text" v-model="configdata.name.value"
            fluid class="border-[1px]" />
          <p-message v-if="nameExists" severity="error" size="small" variant="simple">
            {{ $t('message.warning.configExists') }}
          </p-message>

        </div>
        <div class=".k-desc">
          <label :for="'description'">{{ configdata.description.label }}</label>
          <p-input-text v-if="configdata.description.type === 'text'" id="description" type="text"
            v-model="configdata.description.value" fluid class="border-[1px]" />
        </div>
        <div class=".k-boolValue flex justify-between">
          <label for="boolValue">{{ configdata.boolValue.label }}</label>
          <p-checkbox id="boolValue" binary v-model="configdata.boolValue.value" class="border-[1px]" />
        </div>
        <div class=".k-editable flex justify-between" v-if="!configdata.boolValue.value">
          ...
          <p-checkbox id="editable" binary v-model="configdata.editable.value" class="border-[1px]" />
        </div>
        <div class=".k-multiValue flex justify-between" v-if="!configdata.boolValue.value">
          ...
          <p-checkbox id="multiValue" binary v-model="configdata.multiValue.value" class="border-[1px]" />
        </div>

        <div class=".k-possibleValues" v-if="!configdata.boolValue.value">
          <label for="possibleValues">{{ configdata.possibleValues.label }}</label>
          <SelectSSelect :allow-empty="true" info-id="possibleValues" :editable="true" :multi-selection="true"
            v-model:data="possibleValuesWrapper" v-model:selection="configdata.possibleValues.values" />
        </div>
        <div class=".k-defaultValues" v-if="!configdata.boolValue.value">
          <label for="defaultValues">{{ configdata.defaultValues.label }}</label>
          <SelectSSelect :allow-empty="true" info-id="defaultValues" :multi-selection="configdata.multiValue.value"
            v-model:data="possibleValuesWrapper"
            :selected-options="configdata.multiValue.value ? configdata.defaultValues.values : configdata.defaultValues.value"
            @change="(selection) => {
      ...
      <IconILoading v-if="isLoading || isLoadingNameExists"
...
const possibleValuesWrapper = computed({
  get: () => [...new Set([...configdata.value.possibleValues.values, ...configdata.value.defaultValues.values, configdata.value.defaultValues.value])],
  set: (value: string[]) => {
...
async function save() {
  ...
  const defValue = [...configdata.value.possibleValues.values, ...configdata.value.defaultValues.values, configdata.value.defaultValues.value]
  const request = {
