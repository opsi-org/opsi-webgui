...
export const apiMock = (page: Page, apiPath: string, response: any) => page.route(apiPath, route => route.fulfill({
  status: 200,
  headers: {
...
// }
export const callHistoreVariantById = async (page: Page, folder: string, filename: string, variantId: number|string) => {
...
export const callHistoreVariantByName = async (page: Page, folder: string, filename: string, variantName: string = "default") => {
  const x = `https://localhost:6006/__sandbox.html?storyId=components-${folder}-${filename}&variantId=_${variantName}`
  await page.goto(x)
}
export const callHistoireStory = async (page: Page, folder: string, filename: string) => await page.goto(`https://localhost:6006/story/components-${folder}-${filename}`)
...
export const simpleScreenshotTest = async (
    ...
    const variantNames = await page.locator(options.cssClassVariantName || _cssClassVariantName).allInnerTexts()
    ...
    for (const theme of ["light", "dark"]) {
      ...
      for (let i = 0; i < variantNames.length; i++) {
        ...
        // Take the screenshot
        await expect(await element.screenshot()).toMatchSnapshot(screenshotPrefix+'-'+variantName+'-'+theme+'.png')
      }
    }
}
