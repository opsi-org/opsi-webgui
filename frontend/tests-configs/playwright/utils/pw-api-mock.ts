...
type SameSite = 'None' | 'Strict' | 'Lax' | undefined
export const cookieOpsiconfdSession = [
  {
...
export const pageLogout = async ({ page, expect }: any) => {
  await expect(page).not.toHaveURL('/addons/webgui/app/login')
  console.log('PAGEURL afterEach:', page.url())

  // await page.screenshot({ path: './tests-screenshots/.tmp/opsi-webgui_' + '' + '_.png' })
...
export const mockEveryThing = (test: any, expect: any) => {
  // ///////////////////////////////////////////////////////////////////////////////////
  const mockData = async ({ page }: any) => {
    ...
    apiMock(page, '**/api/user/configuration', {
      user: 'adminuser',
      configuration: {
        read_only: false,
        depot_access: false,
    ...
    apiMock(page, '**/api/opsidata/hosts/groups?**', {
      ...
      clientdirectory: data.groups.clientdirectory,
    })
    ...
    apiMock(page, '**/api/opsidata/products/installation-status', [
      null,
      'successful',
      'failed',
      'unknown',
    ...
    apiMock(page, '**/api/opsidata/products**', data.clientProducts, {
      'access-control-expose-headers': 'x-total-count',
      'x-total-count': data.clientProducts.length,
    })
    apiMock(page, '**/api/opsidata/products/groups**', {
      groups: data.groups.productgroups,
    })
  ...
  // ///////////////////////////////////////////////////////////////////////////////////
  test.beforeEach(async ({ page, context }: any) => {
    // page.setDefaultTimeout(55555)
    await mockData({ page })
