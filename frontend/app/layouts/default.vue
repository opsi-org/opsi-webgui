...
<template>
  <div class="h-screen w-screen flex flex-col overflow-hidden">
    ...
    <LayoutsTopbar :default-page="defaultPage" :package-version="runtimeConfig.public.packageVersion || '1.0.0'"
      :formatted-time="formattedTime" :is-warning="isWarning" :health-check-tooltip="healthCheckTooltip"
      :health-worst-case="currentHealthWorstCase" :health-counts="currentHealthCounts" @toggle-sidebar="toggleSidebar"
    ...
    <div class="flex-1 flex overflow-hidden relative">
      <div v-if="isMobile && sidebarOpen" class="absolute inset-0 bg-black/50 z-30" role="button" tabindex="0"
        :aria-label="String($t('common.close'))" @click="sidebarOpen = false" @keydown.enter="sidebarOpen = false"
      ...
      <main class="flex-1 flex flex-col overflow-hidden bg-(--color-surface)">
        ...
        <div class="shrink-0 z-10 px-2 md:px-2.5">
          ...
          <Transition name="slide-down">
            <CoreAppAlertInline v-if="messageBusStore.certWarning" color="warning" variant="subtle" compact class="mt-2"
              closable @close="messageBusStore.certWarning = false">
    ...
    <Transition name="slide-up">
      <div v-if="quickpanelOpen && useOverlayQuickpanel" class="fixed inset-0 z-50">
        <div class="absolute inset-0 bg-black/50" role="button" tabindex="0" :aria-label="String($t('common.close'))"
          @click="quickpanelOpen = false" @keydown.enter="quickpanelOpen = false"
          ...
          <div class="p-2.5 h-full min-h-0 flex flex-col overflow-hidden">
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <CoreAppButton :icon="icons.x" size="sm" variant="ghost" color="neutral"
                :aria-label="String($t('common.close'))" class="rounded-full" @click="quickpanelOpen = false" />
            </div>
