...
<script setup lang="ts">
  ...
  const routeTab = computed((): string => {
    ...
    return VALID_TABS.includes(normalized as (typeof VALID_TABS)[number]) ? normalized : 'attributes'
  })
  ...
  useHead({
    title: () => (selectedServerId.value ? `${selectedServerId.value} - ${activeTab.value}` : 'Server Configuration'),
  })
</script>
