...
export function useHostConfigPage(onCancelLeave?: () => void) {
	...
	function refresh() { hostConfigTabsRef.value?.refresh?.() }
	function saveAll() { hostConfigTabsRef.value?.saveAll?.() }
	function discardAll() { hostConfigTabsRef.value?.discardAll?.() }
