...
const webguiFeatures = computed(() => {
    ...
    return [
        { key: 'readOnly', i18nKey: 'readOnly', restricted: userConfigData.value?.read_only ?? false, icon: icons.eye, shortLabel: 'Read Only' },
        { key: 'serverWrite', i18nKey: 'serverWrite', restricted: !(userConfigData.value?.server_write_access ?? true), icon: icons.serverStack, shortLabel: 'Server Write' },
        { key: 'depotAccess', i18nKey: 'depotAccess', restricted: userConfigData.value?.depot_access ?? false, icon: icons.server, shortLabel: 'Depot Access' },
        { key: 'clientCreation', i18nKey: 'clientCreation', restricted: !(userConfigData.value?.client_creation ?? true), icon: icons.add, shortLabel: 'Client Create' },
        { key: 'hostGroupAccess', i18nKey: 'hostGroupAccess', restricted: userConfigData.value?.host_group_access ?? false, icon: icons.client, shortLabel: 'Host Groups' },
        { key: 'productGroupAccess', i18nKey: 'productGroupAccess', restricted: userConfigData.value?.product_group_access ?? false, icon: icons.product, shortLabel: 'Product Groups' },
        { key: 'terminal', i18nKey: 'terminal', restricted: terminalDisabled, icon: 'heroicons:command-line', shortLabel: 'Terminal' },
    ]
})
...
const systemInfoItems = computed<SystemInfoItem[]>(() => {
    ...
    if (d.opsiconfd_version)
        items.push({ label: 'opsiconfd', value: String(d.opsiconfd_version) })

    const osRelease = d.os_release as Record<string, unknown> | undefined
    if (osRelease?.PRETTY_NAME)
        items.push({ label: 'os', value: String(osRelease.PRETTY_NAME) })

    const processor = d.processor as Record<string, unknown> | undefined
    if (processor)
        items.push({ label: 'cpu', value: `${processor.cpu_count} cores · ${sysProcessorModel.value}` })
    ...
    if (sysHostname.value)
        items.push({ label: 'hostname', value: sysHostname.value })

    if (sysIsDocker.value !== null)
        items.push({ label: 'docker', value: null, badge: { text: sysIsDocker.value ? 'Yes' : 'No', color: sysIsDocker.value ? 'info' : 'neutral' } })

    if (sysPythonVersion.value)
        items.push({ label: 'python', value: sysPythonVersion.value })

    if (sysLoadAvg.value)
        items.push({ label: 'load', value: sysLoadAvg.value })
