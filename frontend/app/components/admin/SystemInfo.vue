							...
							<div class="flex items-start gap-1.5 px-2 py-1.5 rounded transition-colors hover:bg-(--color-surface-hover) group/leaf"
								...
								<div class="flex-1 min-w-0 flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
									...
									<div class="flex items-center gap-2 flex-1 min-w-0">
										<CoreAppBadge v-if="typeof value === 'boolean'"
											:color="value ? 'success' : 'neutral'" variant="soft" size="xs">
											{{ value ? 'Yes' : 'No' }}
										</CoreAppBadge>
										<span v-else class="text-sm font-medium truncate" :title="String(value)">
											{{ formatValue(value) }}
										</span>
										<CoreAppButton color="primary" variant="soft" size="xs" :icon="icons.copy"
				...
				<template v-for="(values, category) in filteredDiagnosticsData" :key="category">
					<div v-if="typeof values === 'object' && values !== null && Object.keys(values as object).length > 0"
						...
						<div class="flex items-center gap-1.5 px-2 py-1.5 rounded cursor-pointer transition-colors hover:bg-(--color-surface-hover)"
							...
							<span class="text-xs text-(--color-text-muted) opacity-60">{{ Object.keys(values as
								object).length }}</span>
						</div>
										...
										<template v-if="Array.isArray(v)">
											<div v-for="(item, idx) in v" :key="idx" class="tree-node">
												<div class="flex items-start gap-1.5 px-2 py-1 rounded transition-colors hover:bg-(--color-surface-hover)"
													...
													<span
														class="w-5 flex items-center justify-center shrink-0 mt-0.5" />
													<span class="text-sm break-all">{{ typeof item === 'object' ?
														JSON.stringify(item) : String(item) }}</span>
												</div>
											</div>
										</template>
											...
											<div v-for="(sv, sk) in (v as Record<string, unknown>)" :key="sk"
												...
												<div class="flex items-start gap-1.5 px-2 py-1.5 rounded transition-colors hover:bg-(--color-surface-hover) group/deep"
													...
													<div
														...
														<div class="flex items-center gap-2 flex-1 min-w-0">
															<CoreAppBadge v-if="typeof sv === 'boolean'"
																:color="sv ? 'success' : 'neutral'" variant="soft"
																size="xs">
																{{ sv ? 'Yes' : 'No' }}
															</CoreAppBadge>
															<span v-else-if="typeof sv === 'object'"
															...
															<CoreAppButton color="primary" variant="soft" size="xs"
																:icon="icons.copy"
								...
								<div v-else class="tree-node">
									<div class="flex items-start gap-1.5 px-2 py-1.5 rounded transition-colors hover:bg-(--color-surface-hover) group/leaf"
										...
										<div
											...
											<div class="flex items-center gap-2 flex-1 min-w-0">
												<CoreAppBadge v-if="typeof v === 'boolean'"
													:color="v ? 'success' : 'neutral'" variant="soft" size="xs">
													{{ v ? 'Yes' : 'No' }}
												</CoreAppBadge>
