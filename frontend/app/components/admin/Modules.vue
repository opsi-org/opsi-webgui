	...
	<UCard>
		<template #header>
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-2">
					...
					<span class="text-xs text-gray-500">{{ filteredModules.length }} {{ $t('available') }}</span>
				</div>
			</div>
		</template>
		...
		<div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
			<div v-for="module in filteredModules" :key="module"
				...
				<div class="w-10 h-10 rounded-lg flex items-center justify-center" :class="isObsolete(module)
				...
				<div class="flex-1 min-w-0">
					...
					<div class="flex items-center gap-1.5 mt-0.5">
						<UBadge v-if="isObsolete(module)" color="warning" variant="subtle" size="xs">{{ $t('obsolete')
							}}
						</UBadge>
						<UBadge v-else-if="getModuleState(module) === 'free'" color="info" variant="subtle" size="xs">{{
