...
<template>
  <CoreAppCard fill scrollable>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <span class="text-xs text-(--color-text-muted)"
            >{{ filteredModules.length }} {{ $t('mods.available') }}</span
          >
        </div>
      </div>
    </template>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
      ...
      >
        <div
          class="w-8 h-8 rounded-full flex items-center justify-center bg-(--color-success-soft-bg)"
        >
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-1.5 mt-0.5">
            <CoreAppTooltip v-if="isObsolete(module)" :text="$t('mods.obsolete.desc')">
              <CoreAppBadge color="warning" variant="subtle" class="cursor-help"
                >{{ $t('mods.integrated') }}
              </CoreAppBadge>
            </CoreAppTooltip>
            <CoreAppBadge
              v-else-if="getModuleState(module) === 'free'"
              color="info"
              variant="subtle"
              >{{ $t('mods.free') }}</CoreAppBadge
            >
            <CoreAppBadge
              v-else-if="getModuleState(module) === 'licensed'"
              color="success"
              variant="subtle"
              >{{ $t('mods.licensed') }}</CoreAppBadge
            >
            <CoreAppBadge
              v-else-if="getModuleState(module) === 'unlicensed'"
              color="error"
              variant="subtle"
