  ...
  <CoreAppCard fill scrollable>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <span class="text-xs text-(--color-text-muted)">{{ filteredHealthData.length }} {{ $t('diag.checks') }}</span>
      </div>
    </template>
...
<script setup lang="ts">
  const { t: $t } = useI18n()
  defineProps(['filteredHealthData', 'loading', 'stats', 'statusFilter', 'icons', 'expanded', 'getStatusType'])
