...
<template>
  <UTooltip :text="tooltipText">
    <div class="flex flex-col items-center gap-0.5 rounded-lg py-1.5 cursor-help transition-colors hover:bg-(--color-surface-hover)">
      <div class="relative w-9 h-9 flex items-center justify-center overflow-visible">
        ...
        >
          <UIcon :name="restricted ? icons.lock : icons.check" class="w-3 h-3" :class="colorClasses.badgeIcon" />
        </span>
      </div>
