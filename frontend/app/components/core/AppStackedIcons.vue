...
<template>
	<span class="relative inline-flex items-center justify-center shrink-0 overflow-visible">
		...
		<template v-if="secondaryIcon">
			...
			<CoreAppIcon v-else :name="secondaryIcon" class="absolute -bottom-1 -right-1 shrink-0"
...
const secondaryColorClass = computed(() => {
	switch (props.secondaryColor) {
		...
		case 'neutral': return 'text-(--color-text-muted)'
		default: return ''
	}
})
...
const badgeBgClass = computed(() => {
	switch (props.badgeColor) {
		...
		case 'neutral': return 'bg-(--color-text-muted)'
		case 'none': return 'bg-(--color-surface)'
		case 'primary':
		default: return 'bg-opsi-blue'
	}
})
