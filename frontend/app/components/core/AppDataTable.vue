  ...
  <div class="data-table flex flex-col h-full min-h-0 min-w-0" :class="{ 'data-table--compact': isCompactDensity }">
            ...
            <div class="p-2.5 min-w-96 overflow-y-auto bg-(--color-background) rounded shadow-lg">
              ...
              <div class="mb-3 grid grid-cols-[6.5rem_1fr] items-center gap-x-2 gap-y-2.5">
                ...
                <div class="flex gap-0.5">
                  <UButton size="xs" class="flex-1" :color="'primary'" :variant="tableSettings.settings.displayMode === 'infinite' ? 'solid' : 'outline'
                    " @click="changeDisplayMode('infinite')">
                    {{ $t('settings.infiniteScroll') }}
                  </UButton>
                  <UButton size="xs" class="flex-1" :color="'primary'" :variant="tableSettings.settings.displayMode === 'pagination' ? 'solid' : 'outline'
                ...
                <div class="flex items-center gap-1">
                  ...
                  <UButton size="xs" variant="ghost" color="neutral" :icon="tableSettings.settings.sortDirection === 'asc'
                    ? icons.sortAsc
                    : icons.sortDesc
                    " :title="tableSettings.settings.sortDirection === 'asc'
                      ? String($t('common.ascending'))
                      : String($t('common.descending'))
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0 flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden' }"
      ...
      <div ref="tableContainer"
        class="flex-1 min-h-0 min-w-0 overflow-x-auto overflow-y-auto transition-all duration-100" tabindex="0"
        ...
        <div v-else>
          <table class="w-max min-w-full" role="grid">
            <thead class="bg-(--color-surface) sticky top-0 z-30">
              <tr>
                <th v-if="selectable" class="w-9 px-1 py-0.5 text-center whitespace-nowrap bg-(--color-surface)"
                ...
                <th v-for="col in visibleColumns" :key="col.key" :aria-sort="getSortAriaLabel(col.key)"
                  ...
                  :class="[
                    ...
                    col.stickyRight
                      ? 'sticky z-40 bg-(--color-surface) shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.1)]'
                      : '',
                  ]" :style="{
                    width: col.width,
                    ...
                    textAlign: col.align,
                    ...(col.stickyRight
                      ? { right: hasActions ? actionsColWidth + 'px' : '0' }
                      : {}),
                  ...
                  <slot :name="`header-cell-${col.key}` as any" :column="col"
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <template v-if="col.sortable">
                        <UIcon v-if="tableSettings.settings.sortColumn === col.key" :name="tableSettings.settings.sortDirection === 'asc'
                          ? icons.sortAsc
                          : icons.sortDesc
                          " class="w-3 h-3" />
            ...
            <tbody class="data-table-body" :class="{ 'pb-2': displayMode === 'pagination' }">
              <tr v-for="(row, idx) in displayRows" :key="getRowKey(row)" :aria-selected="isSelected(row)" :tabindex="0"
                ...
                <td v-if="selectable" class="px-1 py-0.5 text-center align-middle" role="gridcell"
                ...
                <td v-for="col in visibleColumns" :key="col.key" role="gridcell"
                  ...
                  <slot :name="`cell-${col.key}` as any" :row="row" :value="getNestedValue(row, col.key)" :index="idx">
                    ...
                    <span v-else :class="getCellContentClass(col)">{{
                ...
                <td v-if="hasActions"
                  ...
                  :class="isHighlighted(row)
                    ? 'bg-(--color-row-selected)'
                    : 'bg-(--color-background) group-hover:bg-(--color-surface-hover)'
                    " @click.stop>
                  <div class="flex items-center justify-center gap-1 rounded-md transition-colors" :class="isHighlighted(row)
                    ? 'bg-(--color-primary)/10 ring-1 ring-(--color-primary)/30 px-1'
                    : ''
                    ">
    ...
    <div class="shrink-0 px-1 rounded-b-lg flex items-center justify-between gap-4">
      <span class="text-xs text-(--color-text-muted)">
        ...
        <template v-else>
          {{ $t('common.showing') }} {{ paginationStartIndex + 1 }}-{{
            Math.min(paginationEndIndex, serverTotal)
          }}
      ...
      <div v-if="displayMode === 'pagination' && totalPages > 1" class="flex items-center gap-1">
        ...
        <template v-for="page in visiblePageNumbers" :key="page">
          ...
          <UButton v-else :aria-label="`${$t('common.page')} ${page}` +
            (page === currentPage ? ` (${$t('common.current')})` : '')
            " :variant="page === currentPage ? 'solid' : 'ghost'" :color="page === currentPage ? 'primary' : 'neutral'"
            size="xs" class="min-w-7" @click="goToPage(page as number)">
            {{ page }}
          </UButton>
        </template>
...
const sortBySelection = ref(
  Boolean(props.sortBySelectionEnabled && (props.selectedKeys?.length ?? 0) > 0)
)
...
const visibleColumns = computed(() =>
  props.columns.filter(
    (c) => c.alwaysVisible || tableSettings.isColumnVisible(c.key, props.columns)
  )
)
...
const hasMoreData = computed(() => {
  if (displayMode.value === 'infinite')
    return hasMoreInfiniteData(autoPageStalled.value, props.rows.length, serverTotal.value)
  return false
})
...
function handleSort(column: string) {
  ...
  if (current === column) {
    tableSettings.settings.sortDirection =
      tableSettings.settings.sortDirection === 'asc' ? 'desc' : 'asc'
  } else {
