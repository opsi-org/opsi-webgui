...
<template>
  <LayoutsPageLayout :loading="loading">
    <template #filters>
      ...
      <CoreAppSelectMenu
        v-model="selectedLogTypeValue"
        :placeholder="$t('logs.selectType')"
        :items="LOG_TYPES"
        :loading="loading"
        value-key="value"
        label-key="label"
      ...
      <div v-if="resolvedClientId" class="flex gap-2">
        ...
        <div v-if="logContent.length > 0">
          <CoreAppButton
            ...
            :aria-label="String($t('settings.autoRefresh'))"
        ...
        <div v-if="logContent.length > 0">
          <CoreAppButton
            ...
            :variant="autoScroll && !hasMarker ? 'solid' : 'ghost'"
            :disabled="hasMarker"
            size="sm"
        ...
        <div v-if="selectedLogTypeValue">
          <CoreAppButton
            ...
            color="primary"
            size="sm"
            :aria-label="String($t('common.download'))"
            :title="String($t('common.download'))"
    ...
    <div class="flex flex-col h-full gap-1.5 min-h-0">
      ...
      >
        <CoreAppButton
          ...
          size="xs"
          class="py-0.5! px-1.5!"
          :title="String($t('logs.scroll'))"
          :aria-label="String($t('logs.scroll'))"
        ...
        <CoreAppButton
          ...
          size="xs"
          class="py-0.5! px-1!"
          :title="String($t('logs.clearMarker'))"
          :aria-label="String($t('logs.clearMarker'))"
          @click="clearMarker"
        />
      </div>

      <CoreAppAlertInline
        v-if="logUpdatePending"
        color="info"
        :title="$t('bus.title')"
        :description="$t('bus.logUpdated')"
        class="shrink-0"
      >
        <template #actions>
...
<script setup lang="ts">
  ...
  async function fetchLog() {
    ...
    const requestId = ++fetchLogRequestId.value
  ...
  watch(
    [() => resolvedClientId.value, selectedLogTypeValue],
    ([clientId, logType], [prevClientId, prevLogType]) => {
      if (clientId === prevClientId && logType === prevLogType) return
      logContent.value = []
      error.value = null
      markerLine.value = -1
      logUpdatePending.value = false
