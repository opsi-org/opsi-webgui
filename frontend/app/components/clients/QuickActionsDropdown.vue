      ...
      <CoreAppDropdownMenu v-if="clientIds.length > 0" :items="actionItems">
        ...
        <template #item-leading="{ item }">
          <CoreAppImage
            ...
            :src="menuItem(item).image as string"
            :dark-src="menuItem(item).darkImage"
            :alt="menuItem(item).label"
        ...
        <div v-if="currentAction === 'deployClientAgent'" class="space-y-3 mb-4">
          ...
          <div class="grid grid-cols-3 gap-2 mb-3">
            <CoreAppButton
              ...
              :color="deployOptions.type === os.value ? 'primary' : 'neutral'"
              size="sm"
              class="justify-center"
              @click="deployOptions.type = os.value"
            >
...
<script setup lang="ts">
  ...
  const allActions = [
    ...
    {
      key: 'deployClientAgent',
      icon: icons.deploy,
      image: 'opsi-client-agent.svg',
      darkImage: 'opsi-client-agent-light.svg',
      color: 'text-(--color-success-soft-text)',
    },
  ...
  const renameValidation = computed(() => {
    ...
    if (/^\d/.test(hostname))
      return {
        color: 'error',
        message: String($t('fields.hostname.required')),
        textClass: 'text-(--color-error)',
      }
    const newId = hostname + '.' + renameDomain.value.trim()
    if (props.clientIds.length === 1 && newId === props.clientIds[0])
      return {
        color: 'error',
        message: String($t('clients.validation.exists')),
        textClass: 'text-(--color-error)',
      }
