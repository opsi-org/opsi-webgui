        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2 shrink-0">
            <CoreAppButton
              ...
              size="sm"
              :disabled="!canCreate"
              :title="String($t('clients.create.title'))"
              @click="handleSubmit"
            >
              ...
              <div class="flex-1 flex flex-col items-start gap-1 min-w-0">
                <div class="flex items-center gap-2 w-full">
                  <CoreAppInput
                    ...
                    :disabled="loading"
                    size="sm"
                    :placeholder="String($t('fields.hostname.enter'))"
                    class="flex-1"
                  />
                  <CoreAppInput
                    v-model="domain"
                    :disabled="loading"
                    size="sm"
                    :placeholder="String($t('fields.domainExample'))"
                    class="flex-1"
                  />
                </div>
            ...
            >
              ...
              <div class="flex-1">
                <CoreAppInput
                  ...
                  :disabled="loading"
                  size="sm"
                  :placeholder="String($t('common.description'))"
                  class="w-full"
                />
              </div>
            </div>
            ...
            >
              ...
              <div class="flex-1">
                <CoreAppInput
                  ...
                  :disabled="loading"
                  size="sm"
                  :placeholder="String($t('fields.inventory'))"
                  class="w-full"
                />
              </div>
            </div>
            ...
            >
              ...
              <div class="flex-1">
                <CoreAppInput
                  ...
                  :disabled="loading"
                  size="sm"
                  :placeholder="String($t('fields.ipExample'))"
                  class="w-full"
                />
              </div>
            </div>
            ...
            >
              ...
              <div class="flex-1">
                <CoreAppInput
                  ...
                  :disabled="loading"
                  size="sm"
                  :placeholder="String($t('fields.macExample'))"
                  class="w-full"
                />
              </div>
            </div>
            ...
            >
              ...
              <div class="flex-1">
                <CoreAppTextarea
                  ...
                  size="sm"
                  :rows="3"
                  :placeholder="String($t('common.notes'))"
                  class="flex-1 w-full"
                />
              </div>
            </div>
            ...
            >
              ...
              <div class="flex-1 flex flex-col items-start gap-1 min-w-0">
                <CoreAppSelect
                  ...
                  :aria-label="String($t('depot.title'))"
                  :disabled="loading"
                  size="sm"
                  class="w-full"
                />
              ...
              <div class="flex-1 min-w-0 space-y-1.5">
                ...
                <div class="border border-(--color-border) rounded-md overflow-hidden">
                  <div class="max-h-40 overflow-auto px-1 py-0.5">
                    <div
                      v-for="item in visibleGroupTreeItems"
                      :key="`group-tree-${item.id}`"
                    ...
                    >
                      ...
                      >
                        <CoreAppIcon :name="expandedGroupIds.has(item.id) ? icons.chevronDown : icons.chevronRight" class="w-2.5 h-2.5" />
                      </CoreAppButton>
                ...
                <div class="text-xs text-(--color-text-muted)">{{ form.groups.length }} {{ $t('common.selected') }}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            >
              ...
              <div class="flex-1 flex items-center gap-2 min-w-0">
                <CoreAppSelectMenu
                  v-model="form.netbootProducts"
                  :items="netbootProductOptions"
                  multiple
                  :aria-label="String($t('products.netboot'))"
                  :disabled="loading"
                  size="sm"
                  class="w-full"
                />
            ...
            >
              <span class="text-sm min-w-0 md:w-1/3 break-all flex items-center gap-1.5">
                <CoreAppImage
                  src="opsi-client-agent.svg"
                  dark-src="opsi-client-agent-light.svg"
                  :alt="String($t('clients.deploy'))"
            ...
            <div v-if="form.agentSetup" class="ml-4 border-l-2 border-(--color-border) pl-4 space-y-0">
                ...
                <div class="flex-1">
                  <div class="grid grid-cols-3 gap-2">
                    <CoreAppButton
                      ...
                      :color="form.agentType === os.value ? 'primary' : 'neutral'"
                      size="sm"
              ...
              >
                ...
                <div class="flex-1 flex items-center gap-2 min-w-0">
                  <CoreAppInput
                    v-model="form.agentUsername"
                    :disabled="loading"
                    size="sm"
                    :placeholder="String($t('fields.adminUsername'))"
                    class="w-full"
                  />
                </div>
              </div>
...
<script setup lang="ts">
  ...
  const { t: $t } = useI18n()
  const { createClient, deployClientAgent, getServers, getClientIds, getServersProducts, addClientToGroups, setClientProductActions } =
    useApiHelpers()
  ...
  const groupTreeFiltered = computed<GroupTreeNodeData[]>(() => {
    ...
    const filterNodes = (nodes: GroupTreeNodeData[]): GroupTreeNodeData[] => {
      ...
      for (const node of nodes) {
        ...
        const match = node.id.toLowerCase().includes(query) || node.label.toLowerCase().includes(query)
  ...
  onMounted(async () => {
    ...
    await Promise.all([fetchClientIds(), fetchNetbootProducts(), fetchClientGroups(false, selectionStore.selectedServers)])
  })
