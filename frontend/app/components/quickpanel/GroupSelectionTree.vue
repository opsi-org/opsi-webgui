  ...
  <div class="flex flex-col h-full min-h-0 groups-dense opsi-compact-page">
    <div class="flex items-center gap-1 mb-2 shrink-0">
      ...
      <CoreAppButton
        ...
        variant="outline"
        color="primary"
        :aria-label="`${$t('common.clearAll')} (${selectedCount})`"
        @click="clearAll"
      />
    </div>
        ...
        <div v-for="section in clientSections" :key="section.id" class="mb-2">
          ...
          >
            <div class="flex items-center gap-1.5">
              <CoreAppIcon
                :name="isSectionCollapsed(section.id) ? icons.chevronRight : icons.chevronDown"
...
<script setup lang="ts">
  ...
  const selectionLoadingIds = ref<Set<string>>(new Set())
  ...
  function getNodeById(nodes: GroupTreeNodeData[], id: string): GroupTreeNodeData | null {
  ...
  function hydrateNodeMembers(groupId: string, members: string[]) {
    const node = getNodeById(rawTree.value, groupId)
    if (!node || members.length === 0) return
    node.members = [...members]
    node.memberCount = Math.max(node.memberCount || 0, members.length)
  }

  function memberMatchesQuery(member: string, query: string): boolean {
    if (!query) return true
    if (member.includes(query)) return true
    ...
    for (const node of nodes) {
      ...
      if (node.members) {
        ...
        for (const member of members) {
          if (!memberMatchesQuery(member, childQuery)) continue
          memberItems.push({
            id: member,
            label: member,
            depth: memberDepth,
  ...
  function isBusyGroup(groupId: string): boolean {
    return isGroupLoading(groupId) || selectionLoadingIds.value.has(groupId)
  }
  ...
  async function handleItemClick(item: FlatItem) {
    if (item.isGroup) {
      ...
      try {
        const members = await fetchAllGroupMembers(item.id)
        hydrateNodeMembers(item.id, members)
        _doGroupSelection(item, members)
      } finally {
        const updatedLoading = new Set(selectionLoadingIds.value)
        updatedLoading.delete(item.id)
        selectionLoadingIds.value = updatedLoading
      }
    } else {
  ...
  async function toggleExpand(nodeId: string) {
    ...
    if (props.groupType !== 'client') return
    const node = getNodeById(rawTree.value, nodeId)
    if (!node?.isSpecial) return
    if ((node.members?.length || 0) > 0) return
    try {
      const members = await fetchAllGroupMembers(nodeId)
      hydrateNodeMembers(nodeId, members)
    } catch {
      // Keep the tree responsive even if special-group member fallback fails.
    }
  }
