...
<template>
  <div class="relative" ref="containerRef">
    ...
    <Transition :name="direction === 'up' ? 'dropdown-up' : 'dropdown'">
      <div v-if="open" :class="[
        'absolute right-0 min-w-32 bg-white dark:bg-[var(--color-surface)] border border-gray-200 dark:border-[var(--color-border)] rounded-lg shadow-lg z-50 py-1',
        direction === 'up' ? 'bottom-full mb-1' : 'top-full mt-1'
      ]">
        <button v-for="locale in availableLocales" :key="locale.code" @click="switchTo(locale.code)" type="button"
          class="w-full px-3 py-2 text-left text-sm text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[var(--color-surface-hover)] transition-colors"
