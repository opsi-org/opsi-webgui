...
<template>
  ...
  <CoreAppModal v-model:open="dialogOpen" :dismissible="true"
    :ui="{ content: 'w-[96vw] max-w-[96vw] h-[84vh] max-h-[84vh]' }" data-testid="product-quick-actions-dialog">
    <template #content>
      <CoreAppCard class="h-full min-w-0"
        :ui="{ root: 'h-full flex flex-col', body: 'flex-1 min-h-0 overflow-hidden p-2.5' }">
        <template #header>
        ...
        <div v-else class="flex flex-col gap-2 h-full min-h-0">
          <CoreAppAlertInline v-if="errorMessage" color="error" :description="errorMessage" variant="subtle" closable
          ...
          <div class="shrink-0 max-h-[44%] min-h-0 divide-y divide-(--color-border) overflow-auto pr-1">
                ...
                <div class="flex-1 space-y-2">
                  <div class="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <CoreAppSelect v-model="filters.installationStatus" :items="installationStatusOptions" size="xs"
                        class="w-full" :aria-label="String($t('products.status'))"
                        data-testid="product-quick-actions-installation-status" @update:model-value="fetchPreview" />
                    </div>
                    <div>
                      <span class="text-xs text-(--color-text-muted) block mb-1">
                        {{ $t('actions.result') }}
                      </span>
                      <CoreAppSelect v-model="filters.actionResult" :items="actionResultOptions" size="xs"
                        class="w-full" :aria-label="String($t('actions.result'))"
                        data-testid="product-quick-actions-action-result" @update:model-value="fetchPreview" />
                    </div>
                  </div>
                  <span class="flex items-center gap-2 cursor-pointer">
                    <CoreAppCheckbox v-model="filters.outdatedOnly"
                      :aria-label="String($t('products.outdated.onClient'))"
                      data-testid="product-quick-actions-outdated-only" @update:model-value="fetchPreview" />
              ...
              <div class="form-row flex flex-col md:flex-row items-start gap-y-1 gap-x-4 py-2.5">
                ...
                <div class="flex-1">
                  <CoreAppSelect v-model="actionRequest" :items="actionRequestOptions" size="sm" class="w-full"
                    :aria-label="String($t('actions.request'))" data-testid="product-quick-actions-action-request"
              ...
              <div class="form-row flex flex-col md:flex-row items-start gap-y-1 gap-x-4 py-2.5">
                ...
                <div class="flex-1">
                  <CoreAppSelect v-model="scope" :items="scopeOptions" size="sm" class="w-full"
                    :aria-label="String($t('fields.scope'))" data-testid="product-quick-actions-scope"
                    @update:model-value="fetchPreview" />
          ...
          <div class="flex-1 min-h-0 flex flex-col">
            <div class="flex items-center justify-between mb-1.5">
              ...
              <div class="flex items-center gap-2">
                <CoreAppButton size="xs" variant="outline" color="primary" :icon="icons.refresh"
                  ...
                  data-testid="product-quick-actions-preview-refresh" @click="fetchPreview" />
            ...
            <div class="border border-(--color-border) rounded-lg bg-(--color-surface) flex-1 min-h-0">
              ...
              <CoreAppTable v-else-if="previewData && Object.keys(previewData).length > 0"
                data-testid="product-quick-actions-preview-table" :columns="previewColumns" max-height="100%"
                wrapper-class="h-full min-h-0" :sort-key="previewSortKey" :sort-dir="previewSortDir"
              ...
              <div v-else class="flex justify-center items-center text-xs text-(--color-text-muted) h-full min-h-0">
                --
              </div>
            </div>
          </div>
        ...
        <template #footer>
          <div class="space-y-3">
            ...
            <div class="flex justify-end gap-2">
              <CoreAppButton variant="outline" color="primary" size="sm" @click="resetForm">{{ $t('common.reset') }}
              </CoreAppButton>
              <CoreAppButton v-if="applyResult?.type === 'success'" variant="outline" color="primary" size="sm"
                @click="dialogOpen = false">{{ $t('common.close') }}</CoreAppButton>
              <CoreAppButton v-else color="primary" size="sm" :disabled="isReadOnly || previewData == null || applying"
...
function buildParams(demoMode: boolean): Record<string, unknown> | null {
  ...
  const instStatus =
    filters.value.installationStatus === NOT_APPLIED ? null : filters.value.installationStatus
