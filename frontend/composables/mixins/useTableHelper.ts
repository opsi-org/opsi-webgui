...
export const useTableHelper = (
  ...
  const _firstDummyRow = {
    dummy: true,
    clientId: 'click to load more',
    direction: 'prev',
  }
  ...
  const fetchedDataWrapper = computed<any[]>(() => {
    return tableDataType !== undefined
      ? (fetchedData.value as any)[tableDataType.value]
      : fetchedData.value
  })
  const tableDataWrapper = computed(() =>
    tableDataType !== undefined
      ? (tableData.value as any)[tableDataType.value]
      : tableData.value,
  )

  const maxPage = computed(
    () => Math.ceil(totalItems.value / tableDataWrapper.value.perPage) || -1,
  )

  // watch(()=> tableDataWrapper.value, async ()=>{ await fetch ()}, { deep: true})
  watch(
    () => tableDataWrapper.value.filterQuery,
    async () => {
      await fetch()
    },
    { deep: true },
  ...
  watch(
    () => [tableDataWrapper.value.sortBy, tableDataWrapper.value.sortDesc],
    async () => {
      await fetch()
    },
    { deep: true },
  ...
  async function fetch(location: string | undefined = undefined) {
    ...
    const visiblePages = Math.ceil(
      fetchedDataWrapper.value.filter((x: any) => x.dummy !== true).length /
        tableDataWrapper.value.perPage,
    )
    ...
    if (direction === undefined || direction === '') {
      ...
      if (tableDataWrapper.value.pageNumber < maxPage.value)
        fetchedDataWrapper.value.push(_lastDummyRow)
    } else if (direction === 'next') {
      if (fetchedDataWrapper.value.length > 0) {
        if (fetchedDataWrapper.value[0].dummy)
          fetchedDataWrapper.value.splice(0, 1) // remove first dummy row
        if (fetchedDataWrapper.value[fetchedDataWrapper.value.length - 1].dummy)
          fetchedDataWrapper.value.splice(
            fetchedDataWrapper.value.length - 1,
            1,
      ...
      if (visiblePages >= nPages) {
        // fetchedDataWrapper.value.splice(0, tableDataWrapper.value.perPage) // remove first n rows
        fetchedDataWrapper.value.splice(
          fetchedDataWrapper.value.length - tableDataWrapper.value.perPage,
          tableDataWrapper.value.perPage,
        )
      }
      fetchedDataWrapper.value.unshift(...(await _fetch()))
      if (tableDataWrapper.value.pageNumber !== 1)
        fetchedDataWrapper.value.unshift(_firstDummyRow, _firstDummyRow2)
      if (
        maxPage.value - 1 > 1 &&
        tableDataWrapper.value.pageNumber < maxPage.value - 1
      )
        fetchedDataWrapper.value.push(_lastDummyRow)
    }
  ...
  function updateTableData(v: typeof tableDataWrapper.value) {
    for (const key in v) {
      if (
        JSON.stringify(tableDataWrapper.value[key]) !== JSON.stringify(v[key])
      ) {
        if (v[key]) tableDataWrapper.value[key] = v[key]
      }
    }
