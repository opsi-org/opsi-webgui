...
export const useDynamicHeightTable = (
  ...
  const isLastPage = computed(() => currentPage.value * pageSize.value >= totalItems.value)
  ...
  const visibleTableHeight = computed(() =>
    // fixed height of infiniteScrollDiv (table will have heigher height, so infiniteScrollDiv will have scroll)
    {
      ...
      return actualDataSize.value < pageSize.value / 2 && maxVisibleHeight.value > 1000
  ...
  const tableHeight = computed(() => {
    ...
    const scrollDivCount = isFirstPage.value && isLastPage.value ? 0 : isFirstPage.value ? 1 : 2
  ...
  const pageSize = computed(() => {
    ...
    return Math.min(
      ...
      Math.floor((maxVisibleHeight.value + notTableHeight.value) / rowHeight.value)
    )
  })
