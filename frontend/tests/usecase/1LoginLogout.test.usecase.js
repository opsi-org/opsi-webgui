...
test('Login with incorrect credentials', async ({ page }) => {
  const preRoute = async () => {
    await apiMock(page, '**/auth/login', { http_status: 403, error: '', message: 'My Opsi service authentication error' })
  }
  ...
  await pageLogin(page, null,
    preRoute, postLoginClick,
    'adminuser', 'adminuser-wrong')
})
