...
import { headersToObject, mergeRequestHeaders, shouldSendSessionHeader } from '~/app/utils/requestHeaders'
...
describe('mergeRequestHeaders', () => {
  ...
  it('omits session lifetime header for excluded urls', () => {
    ...
    expect(Object.keys(result).some((k) => k.toLowerCase() === 'x-opsi-session-lifetime')).toBe(false)
  })
  ...
  it('does not duplicate a pre-existing lowercased session header', () => {
    ...
    const sessionKeys = Object.keys(result).filter((k) => k.toLowerCase() === 'x-opsi-session-lifetime')
